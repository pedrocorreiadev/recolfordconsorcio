...
function specialistMessage(specialist: Specialist, name = "") {
  return [
    `Olá, ${specialist.name}.`,
    `Meu nome é ${name || "[seu nome]"} e vi o protótipo de simulação de consórcio.`,
  ].join("\n");
}
  ...
  return (
    <article className="rounded-[24px] border border-[#dce5f0] bg-white p-6 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="rounded-full bg-[#fff4d8] px-3 py-1 text-xs font-bold text-[#976000]">Previsto</span>
      </div>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        ...
        {specialist.email && <Button asChild variant="outline" className="rounded-xl"><a href={mailtoUrl(specialist.email, "Protótipo de simulação", message)}><Mail className="size-4" /> E-mail</a></Button>}
      ...
      <div className="mt-5 flex items-center gap-2 border-t border-[#eef3f9] pt-4 text-xs font-semibold text-[#748398]">
        <UserRoundCheck className="size-4 text-[#b57708]" /> Perfil mantido para mostrar a proposta visual do site
      </div>
    </article>
  );
}
